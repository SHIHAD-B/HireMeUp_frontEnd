            ...
            {selectedJobId ? (
                ...
                <>
                    <div className="w-full h-auto flex-col ">
                            ...
                            <AlertDialogContent>
                                <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>
                                        ...
                                        <div>
                                            <p><strong>Company Name:</strong> {data?.find((item:any) => item._id == selectedApplication?.companyId)?.company_name}</p>
                        ...
                        <div className="lg:w-full  h-auto flex ">
                            <div className="flex-1 w-full h-auto bg-violete-50 flex flex-col pl-2 gap-4">
                                ...
                                <div className="w-full  flex gap-4 ">
                                    <span onClick={() => filter("")} className={` ${active == "" && 'border-b-4 border-customviolet'} lg:text-sm text-xs font-bold cursor-pointer `}>All ({applicants?.length})</span>
                                    <span onClick={() => filter("in-review")} className={`${active == "in-review" && 'border-b-4 border-customviolet'} lg:text-sm text-xs font-bold cursor-pointer`}>In Review ({applicants?.filter((item: any) => item.hiring_status == "in-review").length})</span>
                                    <span onClick={() => filter("shortlisted")} className={`${active == "shortlisted" && 'border-b-4 border-customviolet'} lg:text-sm text-xs font-bold cursor-pointer`}>shortlisted ({applicants?.filter((item: any) => item.hiring_status == "shortlisted").length})</span>
                                    <span onClick={() => filter("interview")} className={`${active == "interview" && 'border-b-4 border-customviolet'} lg:text-sm text-xs font-bold cursor-pointer`}>interview ({applicants?.filter((item: any) => item.hiring_status == "interview").length})</span>
                                    <span onClick={() => filter("hired")} className={`${active == "hired" && 'border-b-4 border-customviolet'} lg:text-sm text-xs font-bold cursor-pointer`}>hired ({applicants?.filter((item: any) => item.hiring_status == "hired").length})</span>
                                    <span onClick={() => filter("rejected")} className={`${active == "rejected" && 'border-b-4 border-customviolet'} lg:text-sm text-xs font-bold cursor-pointer`}>rejected ({applicants?.filter((item: any) => item.hiring_status == "rejected").length})</span>
                                </div>
                                <div className="lg:w-full w-[90%]  overflow-x-scroll shadow-md sm:rounded-lg m-4 ">
                                    <table className="lg:w-[95%]   text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 ">
                                                    ...
                                                    {appliation?.map((item, index) => (
                                                        <tr key={index} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 cursor-pointer">
                                                            <th onClick={() => handleJobClick(String(item.jobId))} scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                                                {data?.find((items:any) => items._id == item.companyId)?.company_name}
                                                            </th>
                                                            <td onClick={() => handleJobClick(String(item.jobId))} className="px-6 py-4">
                                                                {jobs?.find((items:any) => items._id == item.jobId)?.job_title}
                                                            </td>
                                                            <td onClick={() => handleJobClick(String(item.jobId))} className="px-6 py-4">
